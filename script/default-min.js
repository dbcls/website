...
var setData = function setData(data) {
  ...
  feedArray = feedArray.map(function (data) {
    return data.gs$cell;
  }); //行、列の長さ

  var colLength = feedArray.map(function (data) {
    return data.col;
  });
  colLength = Math.max.apply(null, colLength);
  var rowLength = feedArray.map(function (data) {
    return data.row;
  });
  rowLength = Math.max.apply(null, rowLength);
  var services_array = []; //配列にpushするための整形
  ...
  for (var _i = 0; _i < feedArray.length; _i++) {
    var current_cell = feedArray[_i];
...
var initialize = {
  'index': function index() {
    ...
    function slideSwitch() {
      ...
      if ($active.length == 0) $active = $('.main-image__contents DIV:last'); // use this to pull the divs in the order they appear in the markup

      var $next = $active.next().length ? $active.next() : $('.main-image__contents DIV:first'); // uncomment below to pull the divs randomly
  ...
  'post': function post() {
    ...
    marked($('.markdown-body').html()); //タグ名を日本語に変換
  ...
  'references': function references() {
    ...
    $.ajax({
      ...
      success: function success(data) {
        var update_date = new Date(data.feed.updated.$t);
        var year = update_date.getFullYear();
        var month = update_date.getMonth();
        var date = update_date.getDate();
        $('.update_date').text("".concat(year, "\u5E74").concat(month + 1, "\u6708").concat(date, "\u65E5\u73FE\u5728")); // Lading 画像を消す
        ...
        elementArray[0].forEach(function (col_title, i) {
        ...
        for (i = 0; i < names.length; i++) {
          ...
          indivisual_service = indivisual_service.filter(function (data) {
            return data[1] !== 'Original';
          });
          var nameslength = indivisual_service.length;
          element += '<tr><td><div class="filName" data-category="' + names[i] + '">' + names[i] + '</div></td>' + '<td><p class="quote_num">' + nameslength + '</p></td></tr>';
        }
        ...
        function displayIndividual(names) {
          ...
          $('.main__content-title').text(arranged_name); //filterListをarranged_nameのものだけにフィルタリングして新しい配列

          $('.publications__wrapper').empty();
          var service_array = [];
          service_array = filterList[arranged_name];
          service_array = service_array.filter(function (data) {
            return data[1] !== 'Original';
          });
          var results = "";

          for (var i = 0; i < service_array.length; i++) {
            results += '<div class="publications__column__wrapper">' + '<h4 class="publications__column__title">' + service_array[i][title_order] + '</h4>' + '<p class="publications__column__pubmed"><span class="publications__column__title-small">Pubmed: </span><a href="https://www.ncbi.nlm.nih.gov/pubmed/?term=' + service_array[i][2] + '">https://www.ncbi.nlm.nih.gov/pubmed/?term=' + service_array[i][2] + '</a></p>' + '<p class="publications__column__DOI"><span class="publications__column__title-small">DOI: </span><a href="' + service_array[i][DOI_order] + '">' + service_array[i][DOI_order] + '</a></p>' + '<div class="publications__column__wrapper-small">' + '<i class="fa fa-user" aria-hidden="true"></i>' + '<p>' + service_array[i][first_author_order] + '</p>' + '<i class="fa fa-clock-o" aria-hidden="true"></i>' + '<p>' + service_array[i][e_pub_date_order] + '</p>' + '<i class="fa fa-book" aria-hidden="true"></i>' + '<p>' + service_array[i][Journal_order] + '</p>' + '<i class="fa fa-quote-right" aria-hidden="true"></i>' + '<p>' + service_array[i][citation_order] + '</p>' + '</div>' + '</div>';
          }
  ...
  'services': function services() {
    ...
    function servicesFrontDisplay() {
      $.ajax({
        ...
        async: true,
        success: function success(data) {
          var elementArray = data; //column1に"Y"のあるrowをとってくる
          ...
          var application_order = getOrder("category", "Database application developer (REST API, SPARQL)/アプリケーション開発者 (REST API, SPARQL)");
          var data_scientist_order = getOrder("category", "Data scientist/大規模データ解析者 (一括DL)");
          ...
          for (var i = 0; i < symbolYList.length; i++) {
            ...
            function addTagLine(array, lang) {
              ...
              if (lang === 'ja') {
                for (var j = 0; j < array.length; j++) {
                  ...
                  if (category_name === "biologist" || category_name === "application" || category_name === "data-scientist" || category_name === "provider") {
                    user = 'user';
                  }

                  categoryTag += '<div class="service_category card ' + user + ' tag_element ' + array[j] + '">' + tagMapping[category_name].ja + "</div>";
                }
              } else if (lang === 'en') {
                for (var j = 0; j < array.length; j++) {
                  var category_name = array[j];
                  ...
                  categoryTag += '<div class="service_category card ' + _user + ' tag_element ' + array[j] + '">' + tagMapping[category_name].en + "</div>";
                }
              }
            ...
            if (lang === 'ja') {
              element += '<article class="article__section contener-type-box mix ' + tagName + '">' + '<div id="repos_image0" class="repos_image">' + '<a href="' + symbolYList[i][url_order] + '" target="_blank">' + '<img src="./img/service_assets/' + symbolYList[i][image_order] + '" alt="' + symbolYList[i][service_name_order] + '" class="object-fit-img img_services" /></a></div>' + '<div id="repos_name' + i + '" class="repos_name">' + '<p class="name name_ja" id="' + symbolYList[i][service_name_order] + '">' + symbolYList[i][core_service_name_order] + '</p>' + '<div class="keyword"><p>' + symbolYList[i][keyword_ja_order] + '</p></div>' + addTagLine(tagArray, 'ja') + '<div class="btn-box">' + '<a class="page_btn more_btn" target="_blank" href="#' + service_name_hash + '">' + '詳細' + '</a>' + '<a href="' + symbolYList[i][url_order] + '" class="page_btn access_btn" target="_blank">アクセス</a>' + '</div></div>';
            } else if (lang === 'en') {
              element += '<article class="article__section contener-type-box mix ' + tagName + '">' + '<div id="repos_image0" class="repos_image">' + '<a href="' + symbolYList[i][url_order] + '" target="_blank">' + '<img src="./img/service_assets/' + symbolYList[i][image_order] + '" alt="' + symbolYList[i][service_name_order] + '" class="object-fit-img img_services" /></a></div>' + '<div id="repos_name' + i + '" class="repos_name">' + '<p class="name name_en" id="' + symbolYList[i][service_name_order] + '">' + symbolYList[i][service_name_order] + '</p>' + '<div class="keyword"><p>' + symbolYList[i][keyword_en_order] + '</p></div>' + addTagLine(tagArray, 'en') + '<div class="btn-box">' + '<a class="page_btn more_btn" target="_blank" href="#' + service_name_hash + '">' + 'more' + '</a>' + '<a href="' + symbolYList[i][url_order] + '" class="page_btn access_btn" target="_blank">Access</a>' + '</div></div>';
            }
    ...
    function displayRepos(repos_name) {
      $('title').text("".concat(decodeURIComponent(repos_name.replace(/\_/g, ' ')), " | DBCLS"));
      ...
      $.ajax({
        ...
        var services_array_Y = services_array.filter(function (services_array) {
        ...
        md_array_modified.map(function (data) {
          var url = window.location;
          var path = url.href.split('/');
          var file_name = path.pop();
          var lang = '';
  ...
  'events': function events() {
    ...
    $.ajax({
      ...
      success: function success(data) {
        ...
        for (var i = 0; i < events_array.length; i++) {
          elements += '<article class="article__section event__section-ja">' + // '<img src="/img/event_assets/' + events_array[i][event_img_order] + '">' +
          '<div class="article__section__inner">' + '<h4>' + events_array[i][service_name_order] + '</h4>' + '<p>' + events_array[i][explanation_order] + '</p>' + '<a href = "' + events_array[i][url_order] + '" class = "page_btn more_btn" > more </a>' + '</div>' + '</article>';
        }
    ...
    var client_h = document.getElementById('main__contents-event').clientHeight; //クリックでactiveが切り替わる
  ...
  'members': function members() {
    $.when($.getJSON('../json/members.json'), $.getJSON('../json/services.json')).done(function (data, data_services) {
      var lang = $('html').attr('lang');
      var element = "";
      ...
      if (lang === 'ja') {
        ...
        for (var i = 1; i < data.length; i++) {
          ...
          var position = data[i][position_ja_order]; //var position_en = data[i][]

          var keyword = data[i][keyword_order]; //var keyword_en = data[i][]
          ...
          var link_section = judgeExist(mail, 'btn-mail', 'Mail') + judgeExist(github, 'btn-github', 'GitHub') + judgeExist(orcid, 'btn-orcid', 'ORCID') + judgeExist(research_map, 'btn-researchmap', 'researchmap') + judgeExist(googleScholar, 'btn-gs', 'Google Scholar') + judgeExist(website, 'btn-web', 'Website');
        ...
        for (var i = 1; i < data.length; i++) {
          ...
          var name_en = data[i][name_en_order];
          var image = data[i][image_order];
          var position = data[i][position_en_order]; //var position_en = data[i][]
          ...
          var non_publish = data[i][non_publish_order];
          var link_section = '';
          link_section = judgeExist(mail, 'btn-mail', 'Mail') + judgeExist(github, 'btn-github', 'GitHub') + judgeExist(orcid, 'btn-orcid', 'ORCID') + judgeExist(research_map, 'btn-researchmap', 'researchmap', 'en') + judgeExist(googleScholar, 'btn-gs', 'Google Scholar') + judgeExist(website, 'btn-web', 'Website');
          ...
          if (position.match(/Guest/) || position.match(/Visiting/)) {
            element_collaborators += '<div class="content__member" id="' + name_en + '">' + '<div class="repos_image">' + '<img src="./img/member/' + image + '" alt="' + name_en + '" class="img_member"></div>' + '<ul><li class="position">' + position + '</li>' + '<li class="repos_name"><span class="name_ja">' + name_ja + '</span><span class="name_en">' + name_en + '</span></li>' + '<li class="keyword">' + keyword_en + '</li>' + '<li class="PIC">Responsible for：<div class="member-list__services"></div></li>' + '<li class="links"><div class="btn-box">' + link_section + '</div></li></ul></div>';
          } else {
            element += '<div class="content__member" id="' + name_en + '">' + '<div class="repos_image">' + '<img src="./img/member/' + image + '" alt="' + name_en + '" class="img_member"></div>' + '<ul><li class="position">' + position + '</li>' + '<li class="repos_name"><span class="name_ja">' + name_ja + '</span><span class="name_en">' + name_en + '</span></li>' + '<li class="keyword">' + keyword_en + '</li>' + '<li class="PIC">Responsible for：<div class="member-list__services"></div></li>' + '<li class="links"><div class="btn-box">' + link_section + '</div></li></ul></div>';
          }
        }
      }

      $("#member-list").append(element);
      $("#member-list-collaborators").append(element_collaborators); //担当サービスの実装

      data_services = data_services[0];
      data_services = data_services.filter(function (data) {
        return data[1] === "Y";
      });
      var member = [];
  ...
  'access': function access() {
    /***左サイドバーの動作ここから***/
    //変動要素: main__contents-"event"
    var client_h = document.getElementById('main__contents-kashiwa').clientHeight; //クリックでactiveが切り替わる
...
script.addEventListener('load', function () {
  $(function () {
    ...
    children_pages.map(function (child) {
      if (child === pageType) {
        parent_type = page_map[pageType];
      }
    });
    current_class_name = '.' + parent_type;
    $('.header__nav__contents' + current_class_name).find('a').css('border-bottom', '2px solid white'); //header言語切り替え

...
var initialize = {
        ...
        success: function success(data) {
          ...
          for (var i = 0; i < symbolYList.length; i++) {
            ...
            if (file_name === 'services.html') {
              element += '<article class="article__section contener-type-box mix ' + tagName + '">' + '<div id="repos_name' + i + '" class="repos_name">' + '<p class="name name_ja" id="' + symbolYList[i][service_name_order] + '">' + symbolYList[i][core_service_name_order] + '</p>' + '<div class="keyword">' + symbolYList[i][keyword_ja_order] + '</div>' + addTagLine(tagArray, 'ja') + '<div class="btn-box">' + '<a class="page_btn more_btn">' + '詳細' + '</a>' + '<a href="' + symbolYList[i][url_order] + '" class="page_btn access_btn" target="_blank">アクセス</a>' + '</div></div>' + '<div id="repos_image0" class="repos_image">' + '<img src="./img/service_assets/' + symbolYList[i][image_order] + '" alt="' + symbolYList[i][service_name_order] + '" class="object-fit-img img_services"></div>';
            } else if (file_name === 'services-en.html') {
              element += '<article class="article__section contener-type-box mix ' + tagName + '">' + '<div id="repos_name' + i + '" class="repos_name">' + '<p class="name name_en" id="' + symbolYList[i][service_name_order] + '">' + symbolYList[i][service_name_order] + '</p>' + '<div class="keyword">' + symbolYList[i][keyword_en_order] + '</div>' + addTagLine(tagArray, 'en') + '<div class="btn-box">' + '<a class="page_btn more_btn">' + 'more' + '</a>' + '<a href="' + symbolYList[i][url_order] + '" class="page_btn access_btn" target="_blank">Access</a>' + '</div></div>' + '<div id="repos_image0" class="repos_image">' + '<img src="./img/service_assets/' + symbolYList[i][image_order] + '" alt="' + symbolYList[i][service_name_order] + '" class="object-fit-img img_services"></div>';
            }
  ...
  'members': function members() {
    $.when($.getJSON('https://sheets.googleapis.com/v4/spreadsheets/1bSnbUztPDl3nhjQFbScjtTXpQtXOkqZE83NMilziHQs/values/%E7%A0%94%E7%A9%B6%E8%80%85ID?key=AIzaSyA06bNLDsMu3-CvBIwB_oj_dZh7A6qTYMU'), $.getJSON('https://sheets.googleapis.com/v4/spreadsheets/1bSnbUztPDl3nhjQFbScjtTXpQtXOkqZE83NMilziHQs/values/%E3%82%B5%E3%83%BC%E3%83%93%E3%82%B9%E4%B8%80%E8%A6%A7?key=AIzaSyA06bNLDsMu3-CvBIwB_oj_dZh7A6qTYMU')).done(function (data, data_services) {
      var element = "";
      ...
      if (file_name === 'members.html') {
        ...
        for (var i = 1; i < data.values.length; i++) {
          ...
          var link_section = judgeExist(mail, 'btn-mail', 'Mail') + judgeExist(github, 'btn-github', 'GitHub') + judgeExist(orcid, 'btn-orcid', 'ORCID') + judgeExist(googleScholar, 'btn-gs', 'Google Scholar') + judgeExist(website, 'btn-web', 'Website');
          if (non_publish === 'Yes') {
        ...
        for (var i = 1; i < data.values.length; i++) {
          ...
          var non_publish = data.values[i][non_publish_order];
          var link_section = '';
          link_section = judgeExist(mail, 'btn-mail', 'Mail') + judgeExist(github, 'btn-github', 'GitHub') + judgeExist(orcid, 'btn-orcid', 'ORCID') + judgeExist(googleScholar, 'btn-gs', 'Google Scholar') + judgeExist(website, 'btn-web', 'Website');
